...
return ApiResponse.success(data={"count": len(items)})
```
...
缺少 `response_model` 声明，FastAPI 无法自动生成文档，也不会过滤多余字段。

```python
# ✅ 修复
from server.web.api.comments.schema import CommentListResponse
...
Pydantic 的 `validator` / `field_validator` 是做输入校验的正确位置，view 层只负责业务流程。
